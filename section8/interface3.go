...
// 동물의 행동 인터페이스 선언
// 덕타이핑 -> 메서드로만 타입을 판단함(메서드가 다 구현되어 있다면 그 타입이라 볼 수 있다)
...
func main() {
	...
	//개 행동 실행
	act(dog) //dog는 모든 interface 메서드를 구현했으므로 해당 타입으로 볼 수 있음

	//고양이 행동 실행
	act(cat) //cat는 모든 interface 메서드를 구현했으므로 해당 타입으로 볼 수 있음
