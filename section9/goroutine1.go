...
func main() {
	...
	//생성 방법 매우 간단, 리소스 매우 적게 사용 -> 즉, 수많은 고루틴 동시 생성 실행 가능
	//비동기적 함수 루틴 실행(매우 적은 용량 차지) -> 채널을 통해 통신
	//공유메모리 사용 시에 정확한 동기화 코딩 필요 -
	//싱글루틴(메인 스레드 하나)에 비해 항상 빠른 처리 결과는 아니다.

	//멀티 스레드 장점과 단점
	//장점 : 응답성 향상, 자원공유를 효율적으로 활용, 작업이 분리되어 코드 간결
	//단점 : 구현 어려움, 복잡도 향상 -> 테스트 및 디버깅 어려움, 전체 프로세스의 사이드 이펙트 성능 저하, 동기화, 데드락
	//

	exe1() //가정 먼저 실행(일반적인 실행 흐름)
	//exe2() //가정 먼저 실행(일반적인 실행 흐름)
	//exe3() //가정 먼저 실행(일반적인 실행 흐름)

	//예제1
	fmt.Println("Main Routine Start : ", time.Now())
	go exe2() //별도의 실행흐름으로 처리
	go exe3()
